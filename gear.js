  ...
  for (i=0; i<slotArray.length; i++) {
    ...
    for (j=0; j<gear[list].length; j++) {
      ...
      if (filter.includes(item.source) == false) {
        info += "<tr onclick=clickTable('" + slot + "',this);" + extra + "><td><div class='linkCell'><a href='https://tbc.wowhead.com/item=" + item.id + "'></a></div>" + item.name + "</td><td>" + gemstring + "</td><td>" + item.source + "</td><td>" + item.sta + "</td><td>" + item.int + "</td><td>" + item.SP + "</td><td>" + item.ShP + "</td><td>" + item.FiP + "</td><td>" + item.hit + "</td><td>" + item.crit + "</td><td>" + item.haste + "</td><td>" + item.pen + "</td><td></td><td></td></tr>";
        count++;
      }
    }
    if (count == 0)
